...
same($c['candidate_count'], 12, 'Full unreviewed count is separate from the three-recording preview');
...
same(first_case($db)['candidate_count'], 11, 'Rejected recording leaves the full unreviewed count');
same(count(first_case($db)['evidence']), 3, 'Preview refills without disguising the falling total');
...
same(first_case($db)['candidate_count'], 12, 'Undo restores the original unreviewed total');
same(first_case($db,['details'=>true])['candidate_count'],12,'Loading more choices does not change the total');
same(count(first_case($db,['details'=>true])['evidence']),12,'Expanded evidence can show the full small pool');
$partial=first_case($db,['details'=>true,'clip_available'=>function($path){return strpos($path,'case-0.wav')!==false;}]);
same($partial['candidate_count'],12,'Missing audio remains in unreviewed records, not a claimed playable count');
same(count($partial['evidence']),1,'Only surviving audio is offered for listening');
