...
(function () {
  ...
  function render() {
    ...
    $('caseQueue').innerHTML = cases.map(function (c, i) {
      ...
      // The API total includes unreviewed records beyond the loaded preview.
      // Do not label that total "available": some audio may have been purged.
      var unreviewed = Number.isSafeInteger(c.candidate_count) && c.candidate_count >= c.evidence.length ? c.candidate_count : null;
      var recordingCount = unreviewed === null ? 'Unreviewed recordings' : unreviewed.toLocaleString() + ' unreviewed';
      var previewCount = unreviewed === null ? 'Showing ' + c.evidence.length + ' loaded recordings' : 'Showing ' + c.evidence.length + ' of ' + unreviewed.toLocaleString() + ' unreviewed recording' + (unreviewed === 1 ? '' : 's');
      ...
      return '<article class="ui-card review-card case-card" id="case-' + i + '" tabindex="0" data-card="' + i + '" aria-labelledby="case-title-' + i + '">' +
        ...
        (clip ? '<div class="case-recording-meta"><div><span class="case-recording-date">' + esc(dateLabel(clip.date)) + '</span><span class="case-recording-time">' + esc(clip.time) + '</span></div><span class="case-score" title="BirdNET model score, not a guarantee of a correct identification">Model score <strong class="case-score-value">' + pct(clip.score) + '</strong></span></div>' +
          ...
          '<details class="case-recordings" data-expand="recordings"' + (saved.recordings ? ' open' : '') + '><summary>Choose a recording <span title="Unreviewed records for this question and date range. Some audio may be unavailable.">' + esc(recordingCount) + '</span></summary><p class="case-recording-count">' + esc(previewCount) + '</p><fieldset><legend class="case-sr-only">Recording to listen to and review</legend>' + clips + '</fieldset>' + button(i,'evidence','Load more recordings') + '</details>' : '<p class="case-no-audio">No completed, unreviewed audio is available here. No verdict has been inferred.</p>') +
